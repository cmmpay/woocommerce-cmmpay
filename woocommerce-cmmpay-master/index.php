   ...
   */
   class WC_Tech_Autho extends WC_Payment_Gateway 
   {
      ...
      public function __construct(){
         ...
         $this->id               = 'authorize';
         $this->method_title     = __('CMMPay', 'tech');
         ...
         $this->testurl          = 'https://appdev.cmmpay.net/pay';
      ...
      **/
      ...
      {
         echo '<p>'.__('Thank you for your order, please click the button below to pay with CMMPay.', 'tech').'</p>';
      ...
      **/
      function check_authorize_response()
      {
         ...
         if ( count($_POST) ){
            ...
            //$digest = strtoupper(HASH_HMAC('md5',"^".$hashData."^", hex2bin($signatureKey)));
            ...
            if ( $_POST['x_response_code'] != '' &&  ( strtoupper($_POST['x_SHA2_Hash']) ==  $digest ) ){
                  ...
                  if ( $order->get_status() != 'completed'){
                     ...
                     if ( $_POST['x_response_code'] == 1 ){
                        ...
                        if ( $order->get_status() == 'processing' ){
                            ...
                            $order->add_order_note('Payment successful<br/>Ref Number/Transaction ID: '.$_REQUEST['x_trans_id']);
               ...
               $order->add_order_note('SHA hash mismatched for this transaction. Please contact support <a href="http://www.cmmpay.net/">contact support</a> for help.');
            }
      ...
      **/
      public function generate_authorize_form($order_id)
      {
         ...
         $signatureKey  = ($this->signature_key != '') ? $this->signature_key : '';
         $transactionKey      = ($this->transaction_key != '') ? $this->transaction_key : '';
      ...
      'x_currency_code'      => $order->get_currency(),
        ...
        if($this->mode == 'true'){
         ...
         $html_form    = '<form action="'.$processURI.'" method="post" id="authorize_payment_form">' 
               ...
               . '<input type="submit" class="button" id="submit_authorize_payment_form" value="'.__('Pay via CMMPay', 'tech').'" /> <a class="button cancel" href="'.$order->get_cancel_order_url().'">'.__('Cancel order &amp; restore cart', 'tech').'</a>'
               . '<script type="text/javascript">
                  jQuery(function(){
                     jQuery("body").block({
                           message: "<img src=\"'.$woocommerce->plugin_url().'/assets/images/wpspin-2x.gif\" alt=\"Redirecting…\" style=\"float:left; margin-right: 10px;\" />'.__('Thank you for your order. We are now redirecting you to CMMPay to make payment.', 'tech').'",
                           overlayCSS:
                        {
                           background:       "#ccc",
                           opacity:          0.6,
...
function authsim_action_links ( $links ) {
   $authsim_links = array(
      ...
      '<a href="http://talk.cmmpay.net" target="_blank">Support</a>',
      '<a href="https://app.cmmpay.net/login" >Visit Dashboard</a>'
